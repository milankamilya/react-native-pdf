...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _enableAnnotationInteraction = YES;
        _fitPolicy = 2;
        _spacing = 10;
        _singlePage = NO;
...
- (void)didSetProps:(NSArray<NSString *> *)changedProps
{
    if (!_initialed) {
        ...
        if (_pdfDocument && ([changedProps containsObject:@"path"] || [changedProps containsObject:@"enableAnnotationInteraction"])) {
            if (!_enableAnnotationInteraction) {
                for (unsigned long i=0; i<_pdfView.document.pageCount; i++) {
                    PDFPage *pdfPage = [_pdfView.document pageAtIndex:i];
                    for (unsigned long j=0; j<pdfPage.annotations.count; j++) {
                        pdfPage.annotations[j].readOnly = !_enableAnnotationInteraction;
                    }
                }
            }
        }
